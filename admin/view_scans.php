        ...
        <div class="scans-grid">
            <?php foreach ($scans as $scan): ?>
                <div class="scan-card">
                    ...
                    // Normalize legacy path formats in DB:
                    // - uploads/file.jpg
                    // - ../uploads/file.jpg
                    // - ..\uploads\file.jpg
                    // - C:/xampp/htdocs/Pneumonia/uploads/file.jpg
                    ...
                    if (stripos($normalized, 'http://') === 0 || stripos($normalized, 'https://') === 0) {
                        ...
                        if ($uploads_pos !== false) {
                            // Absolute path case: keep only uploads/...
                            $normalized = substr($normalized, $uploads_pos + 1);
                        }

                        if (strpos($normalized, '../uploads/') === 0) {
                            $normalized = substr($normalized, 3); // uploads/...
                        } elseif (strpos($normalized, './uploads/') === 0) {
                            $normalized = substr($normalized, 2); // uploads/...
                        } elseif (strpos($normalized, '/uploads/') === 0) {
                            $normalized = ltrim($normalized, '/'); // uploads/...
                        } elseif (strpos($normalized, 'uploads/') !== 0) {
                            // Unknown/filename-only case
                            $normalized = 'uploads/' . basename($normalized);
                        }

                        // Admin page lives in /admin, so prepend ../ for web path.
                        $img_src = '../' . $normalized;
                    }
                    ?>
                    <img src="<?php echo htmlspecialchars($img_src); ?>" alt="Scan Image">
