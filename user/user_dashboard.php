<?php
session_start();
$isLoggedIn = isset($_SESSION['user_id']) && isset($_SESSION['role']) && $_SESSION['role'] === 'user';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!$isLoggedIn) {
        $error = "Please log in to analyze an X-ray.";
    } elseif (!isset($_FILES['xray_image'])) {
        ...
        if ($file['error'] !== UPLOAD_ERR_OK) {
            ...
            if (!is_chest_xray($file['tmp_name'])) {
                ...
                if (move_uploaded_file($file['tmp_name'], $file_path)) {
                    ...
                    if ($response === false) {
                        $error = "Failed to contact prediction server: $curl_error";
                    } else {
                        ...
                        if ($http_code === 200) {
                            if (isset($result['error'])) {
                                ...
                                $image_file = "../uploads/" . $file_name;  // Correct path for HTML img src from user/ folder
                                $stmt = $conn->prepare("INSERT INTO scans (user_id, image_path, upload_date, outcome, Confidence) VALUES (?, ?, NOW(), ?, ?)");
                                $stmt->execute([$_SESSION['user_id'], $image_path, $prediction, $confidence]);
                            }
                        } else {
                            if (isset($result['error'])) {
                                $error = $result['error'];
                            } else {
                                $error = "Failed to get prediction from server (HTTP $http_code).";
                            }
                        }
                    }
                } else {
...
$show_xray_hint = isset($error) && stripos($error, 'x-ray') !== false;
?>
...
<head>
    ...
    <style>
        /* Minimal page-specific overrides only if needed */
    </style>
</head>
<body>
    <div class="navbar">
        ...
        <div>
            ...
            <?php else: ?>
                <span class="welcome-msg">Welcome, Guest</span>
    ...
    <div class="dashboard-container">
        ...
        <div class="upload-box">
                ...
                <form id="uploadForm" method="post" enctype="multipart/form-data">
                    ...
                    <?php if (!$isLoggedIn): ?>
                        <div class="info-banner">
                            You can explore the interface, but please log in before analyzing an X-ray.
                        </div>
                    <?php endif; ?>
                ...
                <?php if (isset($error)): ?>
                    ...
                    <?php if ($show_xray_hint): ?>
                        <div class="hint-box">
                            <h3>Upload Tips</h3>
                            <ul>
                                <li>Use an actual chest X-ray (front view) captured by an X-ray machine.</li>
                                <li>Image should be mostly grayscale; avoid photos of people, notes, or colored scans.</li>
                                <li>Ensure the lungs and ribs are visible and centered; crop out borders or annotations.</li>
                                <li>Acceptable formats: JPEG/JPG/PNG, max size 5MB.</li>
                            </ul>
                        </div>
                    <?php endif; ?>
            ...
            <div class="info-box">
                <h2> Hospitals for Pneumonia Checkup</h2>
                <p>If you need medical assistance, you can contact:</p>
                <ul>
        ...
        <div class="history-box">
            <div class="history-header">
                ...
                <div class="history-stats">
                    <span>
                        <?php echo $isLoggedIn ? count($scans) . ' scans' : 'Login to view'; ?>
                    </span>
                </div>
            </div>
            <div class="image-grid" id="historyGrid">
                <?php if ($isLoggedIn): ?>
                    ...
                    ?>
                        <div class="image-card">
                            ...
                            <?php 
                            // Fix image path for display from user/ folder
                            $display_path = $scan['image_path'];
                            if (substr($display_path, 0, 3) !== '../' && substr($display_path, 0, 4) !== 'http') {
                                $display_path = '../' . $display_path;
                            }
                            ?>
                            <img src="<?php echo htmlspecialchars($display_path); ?>" alt="X-Ray Scan" loading="lazy">
                ...
                <?php else: ?>
                    <div class="empty-history">
                        <img src="../icons/empty.png" alt="Please log in">
                        <p>Log in to view and manage your X-ray history.</p>
                    </div>
                <?php endif; ?>
            </div>
        </div>
